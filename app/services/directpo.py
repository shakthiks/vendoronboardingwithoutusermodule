...
-- ✅ FIX: ONLY ONE VENDOR ROW (NO DUPLICATES)
OUTER APPLY (
    SELECT TOP 1 NAME
...
-- ✅ DIRECT PO CONDITION (NO RFQ LINK)
AND NOT EXISTS (
    SELECT 1 
    FROM PURCHRFQLINE R 
...
""")
        ...
        return [{
            ...
            "created_date": format_ist_date_only(r[3]),
            "status": map_po_status(r[4]),
            "currency": r[5] 
        } for r in rows]
async def get_direct_po_list():
    return await run_in_threadpool(get_direct_po_list_sync)
...
def get_direct_po_details_sync(purch_id: str):
...
WHERE L.PURCHID = ?
  AND L.ISDELETED = 0""", (purch_id,))
        ...
        return {
                ...
                "rfq_number": header_row[10],  # RFQID
                "status": map_po_status(header_row[2]),
                "issue_date": format_ist_date_only(header_row[1]),
                "confirmed_date": format_ist_date_only(header_row[11]),  

                "delivery_terms": header_row[5],   # DLVTERM
                "delivery_mode": header_row[6],    #  DLVMODE
                "payment_terms": header_row[7],    # PAYMENT
                "payment_mode": header_row[8],     # PAYMMODE
                "delivery_date": format_ist_date_only(header_row[9])  
            },
...
async def get_direct_po_details(purch_id:str):
    return await run_in_threadpool(get_direct_po_details_sync,purch_id)
