...
# from app.utils.vendor_profile import fetch_vendor_profile  
...
def get_admin_po_summary_sync():
    with get_d365_connection() as conn:
        ...
        cur.execute("""
            SELECT 
                COUNT(*) AS total,
                SUM(CASE WHEN P.PURCHSTATUS IN (0,1) THEN 1 ELSE 0 END) AS open_pos,
                SUM(CASE WHEN P.PURCHSTATUS = 2 THEN 1 ELSE 0 END) AS received_pos,
                SUM(CASE WHEN P.PURCHSTATUS = 3 THEN 1 ELSE 0 END) AS invoiced_pos,
                SUM(CASE WHEN P.PURCHSTATUS =4 THEN 1 ELSE 0 END) AS cancelled_pos
            FROM PURCHTABLE P
        ...
        return {
            ...
            "open_pos": row[1] or 0,
            "received_pos": row[2] or 0,
            "invoiced_pos": row[3] or 0,
            "cancelled_pos": row[4] or 0  
        }
    
async def get_admin_po_summary():
    return await run_in_threadpool(get_admin_po_summary_sync)
def get_admin_po_list_sync():
    with get_d365_connection() as conn:
        cur = conn.cursor()
...
async def get_admin_po_list():
    return await run_in_threadpool(get_admin_po_list_sync)
...
# ADMIN PO DETAILS
# ==========================================
def get_admin_po_details_sync(purch_id: str):
    ...
    with get_d365_connection() as conn:
        ...
        for l in cur.fetchall():
            lines.append({
                ...
                "item": l[1],
                "description": l[2] or "-",
                "procurement_category": l[3] or "-",   # ✅ NEW FIELD
        ...
        # for l in cur.fetchall():
            ...
            #     "amount": float(l[6] or 0),
            #     "delivery_date": format_ist_date_only(l[7]) if l[7] else None
            # })
        ...
        # =========================
        cur.execute("""
            SELECT INVOICEID,DUEDATE,INVOICEAMOUNT,INVOICEAMOUNTMST,CURRENCYCODE,INVOICEDATE
        ...
        for inv in rows:
            invoices.append({
                ...
                "invoice_amount": float(inv[2] or 0),
                "invoice_amount_inr": float(inv[3] or 0),
                "currency": inv[4],
...
async def get_admin_po_details(purch_id: str):
    return await run_in_threadpool(get_admin_po_details_sync, purch_id)
